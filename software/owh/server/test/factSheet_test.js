...
describe("Fact sheet", function () {
    ...
    it("prepare FactSheet for NY Minority Health", function (done){
        new factSheet().prepareFactSheet('NY', "Minority Health").then(function (resp) {
            expect(resp.totalGenderPop).to.eql(5715718);
            expect(resp.state).to.eql("NY");
            expect(resp.fsType).to.eql("Minority Health");
            //No race-white data
            expect(resp.race.length).to.eql(4);
            expect(resp.race[0].name).to.eql('Black');
            expect(resp.race[1].name).to.eql('American Indian');
            expect(resp.race[2].name).to.eql('Asian or Pacific Islander');
            expect(resp.race[3].name).to.eql('Hispanic');
            //Bridge Race data
            expect(resp.ageGroups[0].name).to.eql("10-14 years");
            expect(resp.ageGroups[0].bridge_race).to.eql(360317);
            //Infant
            expect(resp.infantMortalityData.infant_mortality).to.eql(458);
            //TB
            expect(resp.tuberculosis[5].displayValue).to.eql("1 (11.4)");
            //std
            expect(resp.stdData[3].disease).to.eql("Early Latent Syphilis");
            expect(resp.stdData[3].data[0].displayValue).to.eql("Suppressed");
            //hiv
            expect(resp.hivAIDSData[3].disease).to.eql("HIV Diagnoses");
            expect(resp.hivAIDSData[3].data[2].name).to.eql("Black or African American");
            expect(resp.hivAIDSData[3].data[2].displayValue).to.eql("1,255 (51.8)");

            //detail mortality
            expect(resp.detailMortalityData[0].causeOfDeath).to.eql("Total (all ages)");
            expect(resp.detailMortalityData[0].data.deaths).to.eql(27441);
            //Natality
            expect(resp.natalityData.births).to.eql(73403);
            expect(resp.natalityData.population).to.eql(5715718);
            //cancer
            expect(resp.cancerData[0].site).to.eql("Breast");
            expect(resp.cancerData[0].count).to.eql(3356);
            expect(resp.cancerData[0].mortality_rate).to.eql(11);
            //yrbs
            expect(resp.yrbs[2].question).to.eql("Currently use marijuana");
            expect(resp.yrbs[2].data).to.eql("19.2%");
            //brfss
            expect(resp.brfss[1].question).to.eql("Adults who are current smokers");
            expect(resp.brfss[1].data).to.eql("10.2%");
            //prams
            expect(resp.prams.pregnantWoment[0].question).to.eql("Smoking cigarettes during the last three months of pregnancy");
            expect(resp.prams.pregnantWoment[0].data).to.eql("7.0%");
            expect(resp.prams.women[0].question).to.eql("With one or more previous live births who reported unintended pregnancy");
