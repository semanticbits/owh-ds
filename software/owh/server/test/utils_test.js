...
describe("Utils", function(){
    ...
    it("get all selected filter options for MCD filter ", function(done){
        var query = {
            "key": "deaths",
            ...
            "allFilters": [
                {
                    ...
                    "selectTitle": "select.label.filter.mcd",
                    "updateTitle": "update.label.filter.mcd",
                    "aggregationKey": "record_axis_condn.path",
                    "groupOptions": [
                        {
                            "key": "row",
                            "title": "Row",
                        ...
                        },
                        {
                            "key": false,
                            "title": "Off",
                            "tooltip": "Select to hide on data table"
                        }
                    ],
        ...
        var allOptions = searchUtils.getAllSelectedFilterOptions(query, 'deaths');
        expect(allOptions['mcd-chapter-10']).to.not.equal(undefined);
        expect(allOptions['mcd-chapter-10'].options.length).to.equal(1);
        expect(allOptions['mcd-chapter-10'].options[0]).to.equal('A00-B99');
        done();
    });

    it("get all selected filter options for UCD filter ", function(done){
        var query = {
            "key": "deaths",
            "tableView": "number_of_deaths",
            "allFilters": [
                {
                    ...
                    "queryKey": "ICD_10_code",
                    "primary": true,
                    "value": ["A00-B99", "C00-D48"],
                    "groupBy": "row",
                    "type": "label.filter.group.ucd",
                    "groupKey": "ucd",
                    "autoCompleteOptions": [],
                    "filterType": "conditions",
                    "selectTitle": "select.label.filter.ucd",
                    "updateTitle": "update.label.filter.ucd",
                    "aggregationKey": "ICD_10_code.path",
                    "groupOptions": [
                        {
                            "key": "row",
                            "title": "Row",
        ...
        var allOptions = searchUtils.getAllSelectedFilterOptions(query, 'deaths');
        expect(allOptions['ucd-chapter-10']).to.not.equal(undefined);
        expect(allOptions['ucd-chapter-10'].options.length).to.equal(2);
        expect(allOptions['ucd-chapter-10'].options[0]).to.equal('A00-B99');
        expect(allOptions['ucd-chapter-10'].options[1]).to.equal('C00-D48');
        done();
    });
